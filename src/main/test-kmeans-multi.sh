...
for i in $(seq ${ITERATIONCOUNT}); do
    ...
    go run kmeans.go master points.txt TEMP/master.socket ${CLUSTERCOUNT} ${ITERATIONCOUNT}|& grep -vF rpc.Register: &

    MASTERPID=$!
    SUBPROCS=${MASTERPID}
    sleep 1 # wait for master to be able to accept requests
    for WORKER in $(seq $WORKERCOUNT); do 
        go run kmeans.go points.txt TEMP/master.socket TEMP/worker.${WORKER}.socket ${CLUSTERCOUNT} ${ITERATIONCOUNT}&
        SUBPROCS="${SUBPROCS} $!"
    done
